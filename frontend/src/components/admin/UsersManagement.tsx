...
export const UsersManagement = () => {
    ...
    const buildUserPayload = () => {
        ...
        if (editFormData.role === "driver" && selectedUser?.driver_services) {
            
            const cleanedServices = selectedUser.driver_services.filter((v) : v is Record<string, any> => v && typeof v === 'object' && v !== null
            );
            if (cleanedServices.length > 0) {
                payload.driver_services = cleanedServices;
            }
            ...
            <table className="min-w-full divide-y divide-gray-100">
              <thead>
                <tr className="bg-[#f8fafc]">
                  <th className="px-6 py-4 text-left text-xs font-bold text-[#697586] uppercase tracking-wider">
                    Email/Name
                  </th>
                  <th className="px-6 py-4 text-left text-xs font-bold text-[#697586] uppercase tracking-wider">
                    Role
                  ...
                  <th className="px-6 py-4 text-right text-xs font-bold text-[#697586] uppercase tracking-wider">
                    Actions
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100 bg-white">
                {loading ? (
                  <tr>
                    <td
                      colSpan={6}
                      className="px-6 py-12 text-center text-[#697586]"
                    >
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        {(() => {
                          ...
                          return (
                            <span
                              className={`inline-flex items-center gap-1.5 text-xs font-bold px-2.5 py-1 capitalize ${paymentDisplay.badgeClassName}`}
                            >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-[#697586]">
                        {(() => {
                          const verificationDisplay =
                            getVerificationDisplay(user);
                          return (
                            <span
                              className={`text-xs font-bold px-2 py-1 capitalize ${verificationDisplay.badgeClassName}`}
                            >
                          ...
                          {!user.is_approved &&
                            (() => {
                              const needsPaymentReminder =
                                requiresRegistrationPaymentBeforeApproval(
                                  user.role,
                                  !!user.is_paid,
                                );
                              return (
                                <button
                                  ...
                                  title={
                                    needsPaymentReminder
                                      ? "Send registration payment reminder"
                                      : "Approve User"
                                  }
                                >
                                  {needsPaymentReminder ? (
              ...
              >
                ...
                <section className="space-y-4">
                  <div className="flex items-center gap-2 px-1">
                    <h4 className="text-[11px] font-extrabold text-[#5e35b1] uppercase tracking-wider">
                      Core Identity
                    </h4>
                  ...
                  <div className="bg-white p-6  border border-slate-200 shadow-sm space-y-6">
                    ...
                    <div className="flex items-center gap-3 text-slate-500">
                      ...
                      <p className="text-xs font-medium">
                        {(() => {
                          const verificationDisplay =
                            getVerificationDisplay(selectedUser);
                          return (
                            <>
                              Verification status:{" "}
                              <span
                                className={`font-bold capitalize ${verificationDisplay.textClassName}`}
                              >
                                {verificationDisplay.label}
                              </span>
                            </>
                    ...
                    <div className="bg-white p-6 border border-slate-200 shadow-sm space-y-4">
                      {buildDocumentEntries(selectedUser).length > 0 ? (
                        ...
                        <div className="text-sm text-slate-500">
                          No registration documents have been uploaded for this
                          user yet.
                        </div>
                      )}
                    </div>
                ...
                {/* Gap 3 — Pending profile update notice */}
                {selectedUser && (selectedUser as any).pending_updates && (
                  <section className="space-y-4">
                    <div className="flex items-center gap-2 px-1">
                    ...
                    <div className="bg-amber-50 border border-amber-200 p-4 space-y-3">
                      <p className="text-xs font-bold text-amber-700 uppercase tracking-wide">
                        ⚠ This user has a pending profile update awaiting
                        approval
                      </p>
                      <p className="text-xs text-amber-600">
                        The following fields have been submitted for review:
                      </p>
                      <div className="flex flex-wrap gap-2">
                        {Object.keys((selectedUser as any).pending_updates).map(
                          (key) => (
                            <span
                              key={key}
                              className="inline-flex items-center px-2.5 py-1 text-[11px] font-bold bg-white text-amber-700 border border-amber-200"
                            >
                              {key
                                .replace(/_/g, " ")
                                .replace(/url$/, "")
                      ...
                      <p className="text-xs text-amber-500">
                        Review and approve this update in the Pending Updates
                        section.
                      </p>
                    </div>
                  </section>
                          ...
                          <div className="space-y-2">
                            ...
                            <Input
                              ...
                              onChange={(e) =>
                                setEditFormData({
                                  ...editFormData,
                                  professional_body: e.target.value,
                                })
                              }
                              className="font-bold"
                            />
                            ...
                            {professionalServices.map((s, i) => (
                              <div key={i} className="flex gap-2">
                                <Input
                                  value={s}
                                  onChange={(e) =>
                                    updateProfessionalService(i, e.target.value)
                                  }
                          ...
                          {providerServices.map((s, i) => (
                            <div key={i} className="flex gap-2">
                              <Input
                                value={s}
                                onChange={(e) =>
                                  updateProviderService(i, e.target.value)
                                }
                                className="font-bold flex-1"
                              />
                              <Button
                    ...
                    <section className="space-y-4 animate-in slide-in-from-top-2 duration-300">
                      ...
                      <div className="space-y-6 bg-white p-6  border border-slate-200 shadow-sm">
                        {selectedUser?.driver_compliance && (
                          <div
                            className={`border px-4 py-3 ${selectedUser.driver_compliance.ready_for_approval ? "border-emerald-200 bg-emerald-50" : "border-amber-200 bg-amber-50"}`}
                          >
                            <div className="flex items-center justify-between gap-3">
                              <div>
                                ...
                                <p
                                  className={`mt-1 text-sm font-bold ${selectedUser.driver_compliance.ready_for_approval ? "text-emerald-700" : "text-amber-700"}`}
                                >
                                  {selectedUser.driver_compliance
                                    .ready_for_approval
                                    ? "Driver compliance pack is complete."
                                    : "Driver compliance pack is incomplete."}
                                </p>
                              </div>
                              <Badge
                                className={
                            ...
                            {!selectedUser.driver_compliance
                              ...
                              <div className="mt-3 flex flex-wrap gap-2">
                                {selectedUser.driver_compliance.missing_field_labels.map(
                                  (label) => (
                                    <span
                                      key={label}
                                      className="inline-flex items-center px-2.5 py-1 text-[11px] font-bold bg-white text-amber-700 border border-amber-200"
                          ...
                          <div className="space-y-2">
                            ...
                            <Input
                              ...
                              onChange={(e) =>
                                setEditFormData({
                                  ...editFormData,
                                  vehicle_disk_expiry: e.target.value,
                                })
                              }
                              className="font-bold"
                            />
                          ...
                          <div className="space-y-2 md:col-span-2">
                            ...
                            <Input
                              ...
                              onChange={(e) =>
                                setEditFormData({
                                  ...editFormData,
                                  operating_areas: e.target.value,
                                })
                              }
                        ...
                        <div className="space-y-3">
                          ...
                          {Array.isArray(selectedUser?.driver_services) &&
                          selectedUser?.driver_services.length > 0 ? (
                            <div className="grid grid-cols-1 gap-4">
                              {selectedUser.driver_services.map(
                                (vehicle: any, index: number) => {
                                  ...
                                  return (
                                    <div
                                      key={`${vehicle?.registration_number || vehicle?.plate || index}`}
                                    ...
                                    >
                                      ...
                                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                                        ...
                                        <div>
                                          ...
                                          <p className="mt-1 font-semibold text-slate-700">
                                            {vehicle?.disk_expiry ||
                                              selectedUser?.vehicle_disk_expiry ||
                                              "—"}
                                          </p>
                                        </div>
                                      </div>
                                      ...
                                      {images.length > 0 && (
                                        // ── Vehicle image cards ──────────────
                                        // Matches the same card style as the
                                        // Registration Documents section above:
                            ...
                            <div className="border border-dashed border-slate-200 bg-slate-50 px-4 py-6 text-sm text-slate-500">
                              No structured vehicle record is attached to this
                              driver yet.
                            </div>
                          )}
                        </div>
                  ...
                  <div className="p-6 bg-white border border-slate-200 shadow-sm ">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="flex items-center space-x-3 p-3 bg-white  border border-slate-100 shadow-sm">
                        ...
                        <div className="space-y-0.5">
                          <Label
                            htmlFor="is_approved_status"
                            className="text-xs font-bold text-slate-700 cursor-pointer"
                          >
                            Account Approved
                          </Label>
                          <p className="text-[9px] text-slate-500 font-medium">
                            {approvalBlockedByPayment
        ...
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This action <strong>cannot be undone</strong>. Are you sure you
                want to permanently delete{" "}
                <strong>{selectedUser?.email}</strong> and all their data?
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
