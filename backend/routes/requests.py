...
def accept_request(request_id):
    ...
    try:
        ...
        if service_request.request_type != 'cab' and service_request.requester:
            try:
                EmailService.send_request_accepted_email(service_request.requester, service_request)
            except Exception as notify_error:
                current_app.logger.warning(
                    "Request accepted notification failed for %s: %s",
                    service_request.id,
                    notify_error,
                )
